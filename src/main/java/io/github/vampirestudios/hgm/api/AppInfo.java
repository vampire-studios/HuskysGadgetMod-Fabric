...
public class AppInfo {
    ...
    public AppInfo(Identifier appIdentifier, boolean isSystemApp) {
    ...
    public void reload() {
        resetInfo();
        InputStream stream = HuskysGadgetMod.class.getResourceAsStream("/data/" + APP_ID.getNamespace() + "/apps/" + APP_ID.getPath() + ".json");
    ...
    public static class Deserializer implements JsonDeserializer<AppInfo> {
        ...
        public AppInfo deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) {
            try {
                ...
                HuskysGadgetMod.LOGGER.error("Malformed app info json for '" + info.getFormattedId() + "'");
            }
        ...
        private String convertToLocal(String s) {
            ...
            while (m.find()) {
                ...
                s = s.replace(found, I18n.translate("app." + info.getFormattedId() + "." + found.substring(2, found.length() - 1)));
            }
            return s;
        }
    }
