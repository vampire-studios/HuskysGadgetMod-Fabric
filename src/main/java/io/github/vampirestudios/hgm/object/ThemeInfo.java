...
public class ThemeInfo {
    ...
    private transient final Identifier themeID;
    ...
    public ThemeInfo(Identifier identifier) {
    ...
    public void reload() {
        resetInfo();
        InputStream stream = ModSetup.class.getResourceAsStream("/assets/" + themeID.getNamespace() + "/themes/" + themeID.getPath() + ".json");
    ...
    public static class Deserializer implements JsonDeserializer<ThemeInfo> {
        ...
        public ThemeInfo deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) {
            try {
                ...
                HuskysGadgetMod.LOGGER.error("Malformed theme info json for '" + info.getFormattedId() + "'");
            }
